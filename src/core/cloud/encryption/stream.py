...
def encrypt_snapshot(
    ...
    *,
    ...
    """
    if frame_size <= 0:
        raise FormatError(f"frame_size 必须为正数，实际为 {frame_size}")

    plaintext_size = source.stat().st_size
    frames = _frame_count(plaintext_size, frame_size)
    if frames > MAX_FRAMES:
        raise FormatError(f"快照需要 {frames} 帧，超过 {MAX_FRAMES} 的上限")
...
def _read_exactly(handle: BinaryIO, count: int, what: str) -> bytes:
    ...
    if len(data) != count:
        raise IntegrityError(f"快照已被截断：应为 {what} {count} 字节，实际只有 {len(data)} 字节")
    return data
...
def _parse_header(handle: BinaryIO) -> tuple[dict[str, object], bytes]:
    ...
    if len(preamble) != _PREAMBLE.size:
        raise FormatError("不是有效的 YASB 快照：文件过短")

    magic, version, _reserved, header_len = _PREAMBLE.unpack(preamble)
    if magic != MAGIC:
        raise FormatError("不是有效的 YASB 快照：标识错误")
    if version != FORMAT_VERSION:
        raise FormatError(f"不支持的快照格式版本 {version}；当前程序仅支持 {FORMAT_VERSION}")
    if not 0 < header_len <= MAX_HEADER_BYTES:
        raise FormatError(f"快照头部长度 {header_len} 超出范围")

    raw = _read_exactly(handle, header_len, "header")
    try:
        header = json.loads(raw)
    except ValueError as exc:
        raise FormatError(f"快照头部不是有效的 JSON：{exc}") from exc
    if not isinstance(header, dict):
        raise FormatError("快照头部必须是 JSON 对象")
...
def _require_int(header: dict[str, object], field: str, low: int, high: int) -> int:
    ...
    if not isinstance(value, int) or isinstance(value, bool):
        raise FormatError(f"快照头部字段 {field!r} 必须是整数")
    if not low <= value <= high:
        raise FormatError(f"快照头部字段 {field!r} 的值为 {value}，不在 {low}..{high} 范围内")
    return value
    ...
    try:
        with source.open("rb") as src:
            ...
            if header.get("alg") != ALGORITHM:
                raise FormatError(f"不支持的快照算法 {header.get('alg')!r}")
            ...
            if not isinstance(prefix_hex, str):
                raise FormatError("快照头部字段 'nonce_prefix' 必须是字符串")
            try:
                nonce_prefix = bytes.fromhex(prefix_hex)
            except ValueError as exc:
                raise FormatError(f"快照头部字段 'nonce_prefix' 不是有效的十六进制：{exc}") from exc
            if len(nonce_prefix) != NONCE_PREFIX_LEN:
                raise FormatError(f"Nonce 前缀必须为 {NONCE_PREFIX_LEN} 字节，实际为 {len(nonce_prefix)} 字节")

            # A header that disagrees with itself is rejected before any frame is read.
            if frames != _frame_count(plaintext_size, frame_size):
                raise FormatError("快照头部的帧数与其声明的明文大小不符")
            ...
            with GcmKey(file_key) as key, staging.open("wb") as dst:
                for index in range(frames):
                    ...
                    if not TAG_LEN <= frame_len <= frame_size + TAG_LEN:
                        raise FormatError(f"第 {index} 帧声明了不合理的长度 {frame_len} 字节")
            ...
            if src.read(1):
                raise IntegrityError("快照在最后一帧之后仍有多余数据")
            if recovered != plaintext_size:
                raise IntegrityError(f"快照解密后为 {recovered} 字节，而头部声明为 {plaintext_size} 字节")
