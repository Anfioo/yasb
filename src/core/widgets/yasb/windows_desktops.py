...
class WorkspaceButton(QPushButton):
    ...
    def _show_context_menu(self):
        ...
        if app_view:
            ...
            act_move_here = QAction("将窗口移到这里", self)
            ...
            move_menu = QMenu("将窗口移动到", self.window())
    ...
    def set_wallpaper(self):
        image_path, _ = QFileDialog.getOpenFileName(
            self, "选择壁纸图片", "", "Images (*.png *.jpg *.jpeg *.bmp *.gif)"
        )
        if image_path:
            try:
