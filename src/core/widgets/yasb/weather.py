...
class WeatherWidget(BaseWidget):
    ...
    def _popup_card(self):
        ...
        today_label2 = QLabel(
            ...
            f"{'<br>预警将于 ' + self._weather_data['{alert_end_date}'] + ' 到期' if self._weather_data['{alert_end_date}'] else ''}"
...
def get_weather(code: int, day: bool) -> tuple[str, str]:
    ...
    if code in sunny_codes:
        if day:
            return "sunnyDay", "晴"
        return "clearNight", "晴朗"
    if code in cloudy_codes:
        return f"cloudy{time}", "多云"
    if code in foggy_codes:
        return f"foggy{time}", "雾"
    if code in rainy_codes:
        return f"rainy{time}", "雨"
    if code in snowy_codes:
        return f"snowy{time}", "雪"
    if code in thunderstorm_codes:
        return f"thunderstorm{time}", "雷暴"
    if code in blizzard_codes:
        return f"blizzard{time}", "暴风雪"
