...
class TodoWidget(BaseWidget):
    ...
    def _show_sort_menu(self):
        ...
        sort_by_date = QAction("按日期排序（最新）", self)
        sort_by_date_old = QAction("按日期排序（最早）", self)
        ...
        show_all_action = QAction("显示全部分类", self)
    ...
    def _refresh_task_list(self, layout, tasks=None):
        ...
        if tasks:
            ...
            if self._category_filter:
                ...
                category_label = f" 在 <b>{cat_conf.label if cat_conf else cat_key}</b> 中"

            if not self._show_completed:
                msg = (
                    f"还没有任务{category_label}。<br>点击 <b>新任务</b> 创建你的第一个任务！"
                    if not category_label
                    else f"还没有任务{category_label}。<br>点击 <b>新任务</b> 创建你的第一个任务！"
                )
            else:
                msg = f"还没有完成的任务{category_label}。"
    ...
    def _show_add_task_dialog(self):
        self._show_task_dialog(dialog_title="新建任务", save_button_text="添加任务", on_save=self._add_new_task)

    def _show_edit_task_dialog(self, task):
        self._show_task_dialog(
            dialog_title="编辑任务", save_button_text="保存修改", on_save=self._edit_task, task=task
        )
            ...
            try:
                ...
                if delta.days == 0:
                    if delta.seconds < 60:
                        ...
                        friendly_date = f"{minutes} 分钟前"
                    else:
                        hours = delta.seconds // 3600
                        friendly_date = f"{hours} 小时前"
                elif delta.days == 1:
                    friendly_date = "昨天"
                elif 1 < delta.days < 30:
                    friendly_date = f"{delta.days} 天前"
                else:
