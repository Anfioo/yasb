...
class AppDialog(QDialog):
    ...
    def __init__(
        ...
        self.setWindowTitle(title or ("编辑应用" if self.is_edit_mode else "添加应用"))
        ...
        self.title_edit.setPlaceholderText("输入应用标题...")
        ...
        if not show_only_group:
            ...
            self.path_edit.setPlaceholderText("应用可执行文件、命令或网址...")
        ...
        self.group_edit.setPlaceholderText("组名..." if show_only_group else "分组（可选）...")
                ...
                if name.lower() == text.lower():
                    if path.startswith("CPL::"):
                        ...
                        if shell_icon:
                            ...
                            self.icon_edit.setText("")
                            self._show_warning("找不到此 UWP 应用的图标。")
                        ...
                        if os.path.isfile(path):
                            ...
                            if icon_path:
                                ...
                                self._show_warning("提取所选应用的图标失败。")
                        else:
                            self._show_warning("未找到所选应用的有效图标源。")
                    break
    ...
    def browse_path(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "选择应用程序", "", "Executable or Shortcut (*.exe *.lnk);;All Files (*)"
        )
        if file_path:
    ...
    def browse_icon(self):
        icon_path, _ = QFileDialog.getOpenFileName(
            self, "选择图标", "", "Image Files (*.png *.jpg *.jpeg *.gif *.bmp *.ico *.svg);;All Files (*)"
        )
        if icon_path:
            self.icon_edit.setText(icon_path)
...
class LaunchpadWidget(BaseWidget):
    ...
    def _show_context_menu(self, pos, app_data=None, parent_widget=None, event=None):
        ...
        add_action = QAction("添加应用", menu_parent)
        ...
        exit_action = QAction("退出启动台", menu_parent)
    ...
    def _handle_file_drop(self, file_path, refresh_grid=True):
        ...
        if ext == ".lnk":
            ...
            if not app_name or not target_path:
                self._warning_dialog(f"解析快捷方式失败：{file_path}")
                return
            if not icon_path:
            ...
            if not icon_png:
                self._warning_dialog(
                    f"提取应用程序图标失败<br><b>{file_path}</b><br>请手动选择图标。"
                )
            ...
            if not title:
                self._warning_dialog(f"获取可执行文件的描述失败：{file_path}")
                return
    ...
    def _populate_grid(self, search_text: str = ""):
        ...
        if not filtered_apps:
            no_apps_label = QLabel(
                f"未找到应用程序<div style='font-size:14pt;margin-top:12px;font-weight:400'>按 <b>{self._shortcuts['add_app']}</b> 添加新应用</div>"
            )
