...
class CodexUsageWidget(BaseWidget):
    ...
    def _refresh(self) -> None:
        if is_valid_qobject(self._menu) and self._menu.isVisible() and is_valid_qobject(self._refresh_button):
            ...
            self._set_refresh_feedback("正在刷新…", "busy")
    ...
    def _build_overview_page(self) -> QFrame:
        ...
        if self.config.menu.show_overview:
            ...
            for column, (label, key) in enumerate(
                (("今天", "today"), ("7 天", "week"), ("30 天", "month"), ("今年", "year"))
            ):
                name = QLabel(label)
    ...
    def _build_activity_page(self) -> QFrame:
        ...
        previous.setAccessibleName("上个月")
        set_tooltip(previous, "上个月")
        ...
        next_button.setAccessibleName("下个月")
        set_tooltip(next_button, "下个月")
    ...
    def _sync_heatmap(self) -> None:
        ...
        if is_valid_qobject(self._heatmap_history_note):
            self._heatmap_history_note.setText("本月没有本地令牌历史")
        ...
        for index, cell in enumerate(self._heatmap_cells):
            ...
            tooltip = (
                f"{cell_date.isoformat()}：无本地令牌历史"
                if cell_date < history_start
                else f"{cell_date.isoformat()}：{self._format_tokens(value)} 个令牌"
            )
    ...
    def _build_pager(self) -> QFrame:
        ...
        previous.setAccessibleName("上一页 Codex 用量")
        ...
        next_button.setAccessibleName("下一页 Codex 用量")
    ...
    def _sync_pager(self) -> None:
        ...
        self._page_indicator.setAccessibleName(
            f"{titles[self._current_page]} 页，共 {len(visible_pages)} 页中的第 {index + 1} 页"
        )
    ...
    def _sync_reset_credits(self) -> None:
        ...
        suffix = "个重置额度" if available_count == 1 else "个重置额度"
        ...
        if is_valid_qobject(self._reset_credits_empty):
            if credits is None and available_count:
                message = f"{available_count} 个重置额度可用；详情不可用"
            else:
                message = "没有可用的重置额度"
        ...
        for index, widgets in enumerate(self._reset_credit_widgets):
            ...
            expiration = self._fmt_credit_date(credit.get("expires_at"), "过期")
    ...
    def _sync_menu(self) -> None:
        ...
        for name, fallback in (("primary", "主窗口"), ("secondary", "次窗口")):
            ...
            widgets["used"].setText(f"{self._percent(window.get('used'))}% 已用")
            widgets["remaining"].setText(f"{self._percent(remaining)}% 剩余")
            widgets["remaining"].setProperty("class", f"remaining {level}")
            widgets["reset"].setText(f"{self._fmt_reset(window.get('resets_at'))} 后重置")
