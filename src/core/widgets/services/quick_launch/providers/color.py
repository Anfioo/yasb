...
class ColorProvider(BaseProvider):
    ...
    def get_results(self, text: str, **kwargs) -> list[ProviderResult]:
        ...
        if not query:
            return [
                ...
                ProviderResult(
                    title="对比度检查",
                    description="输入：contrast #fff vs #333",
                    icon_char=ICON_COLOR,
                    provider=self.name,
                ...
                ProviderResult(
                    title="混合 / 调和",
                    description="输入：mix #ff0000 + #0000ff",
                    icon_char=ICON_COLOR,
                    provider=self.name,
                ...
                ProviderResult(
                    title="变亮 / 变暗",
                    description="输入：lighten #ff6347 20 或 darken #ff6347 20",
                    icon_char=ICON_COLOR,
                    provider=self.name,
        ...
        if _RANDOM_RE.match(query):
            ...
            for _ in range(20):
                ...
                results.append(
                    ProviderResult(
                        title=rand_hex,
                        description=f"rgb({rr}, {rg}, {rb}) - 按回车查看全部转换",
        ...
        if parsed is None:
            return [
                ProviderResult(
                    title="无效颜色",
                    description="试试：#FF550090、rgba(255,85,0,0.5)、hsl(20,100,50)、lab(50,0,0)、oklab(0.5,0,0)",
                    icon_char=ICON_COLOR,
                    provider=self.name,
                )
            ]
    ...
    def _contrast_results(self, text1: str, text2: str) -> list[ProviderResult]:
        ...
        if c1 is None or c2 is None:
            ...
            return self._parse_error(bad, "用法：contrast #fff vs #333")
        r1, g1, b1, _ = c1
        r2, g2, b2, _ = c2
        ...
        results = [
            ...
            ProviderResult(
                title=hex1,
                description=f"rgb({r1}, {g1}, {b1}) - 按回车查看转换",
            ...
            ProviderResult(
                title=hex2,
                description=f"rgb({r2}, {g2}, {b2}) - 按回车查看转换",
    ...
    def _mix_results(self, text1: str, text2: str, steps_str: str | None) -> list[ProviderResult]:
        ...
        if c1 is None or c2 is None:
            bad = text1 if c1 is None else text2
            return self._parse_error(bad, "用法：mix #ff0000 + #0000ff")
        r1, g1, b1, _ = c1
        r2, g2, b2, _ = c2
        ...
        for mr, mg, mb, m_hex, pct in mixed:
            ...
            results.append(
                ProviderResult(
                    title=m_hex,
                    description=f"rgb({mr}, {mg}, {mb}) - {pct}% 混合 - 按回车查看转换",
                    icon_char=swatch,
                    provider=self.name,
    ...
    def _adjust_results(self, color_text: str, amount: int, lighten: bool) -> list[ProviderResult]:
        ...
        if parsed is None:
            return self._parse_error(color_text, "用法：lighten #ff6347 20 或 darken red 30")
        r, g, b, _ = parsed
    ...
    def _harmony_results(self, color_text: str) -> list[ProviderResult]:
        ...
        parsed = _parse_color(color_text)
        if parsed is None:
            return self._parse_error(color_text, "用法：harmony #ff6347")
        r, g, b, _ = parsed
        orig_hex = _rgb_to_hex(r, g, b)
        results: list[ProviderResult] = [
            ProviderResult(
    ...
    def _blindness_results(self, color_text: str) -> list[ProviderResult]:
        ...
        parsed = _parse_color(color_text)
        if parsed is None:
            return self._parse_error(color_text, "用法：blind #ff6347")
        r, g, b, _ = parsed
        orig_hex = _rgb_to_hex(r, g, b)
