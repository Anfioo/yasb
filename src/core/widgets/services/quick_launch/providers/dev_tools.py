...
_TOOLS: dict[str, dict[str, str]] = {
    ...
    "hash": {
        "name": "哈希生成器",
        "description": "文本的 MD5、SHA1、SHA256、SHA512 哈希",
    },
    "base64": {
        "name": "Base64 编码/解码",
...
class DevToolsProvider(BaseProvider):
    ...
    def _base64_results(self, arg: str) -> list[ProviderResult]:
        if not arg:
            return [self._make_result("在 'base64' 后输入文本", "编码为 Base64。粘贴 Base64 可解码。", "")]
        results: list[ProviderResult] = []
        encoded = base64.b64encode(arg.encode("utf-8")).decode("ascii")
        results.append(self._make_result(encoded, "Base64 编码 - 点击复制", encoded))
        try:
            decoded = base64.b64decode(arg).decode("utf-8")
            results.append(self._make_result(decoded, "Base64 解码 - 点击复制", decoded))
        except Exception:
            pass
        return results
    ...
    def _url_results(self, arg: str) -> list[ProviderResult]:
        if not arg:
            return [self._make_result("在 'url' 后输入文本", "对字符串进行 URL 编码/解码", "")]
        results: list[ProviderResult] = []
        encoded = urllib.parse.quote(arg, safe="")
        results.append(self._make_result(encoded, "URL 编码 - 点击复制", encoded))
        try:
            decoded = urllib.parse.unquote(arg)
            if decoded != arg:
                results.append(self._make_result(decoded, "URL 解码 - 点击复制", decoded))
        except Exception:
            pass
        return results
    ...
    def _jwt_results(self, arg: str) -> list[ProviderResult]:
        if not arg:
            return [self._make_result("在 'jwt' 后粘贴 JWT 令牌", "解码负载（不验证）", "")]
        try:
            parts = arg.split(".")
            if len(parts) < 2:
                return [self._make_result("无效的 JWT", "应为 header.payload.signature 格式", "")]
            payload_b64 = parts[1]
            ...
            results: list[ProviderResult] = []
            results.append(self._make_result("JWT 负载", "点击复制解码后的 JSON", pretty))
            ...
            return [self._make_result("JWT 解码失败", "请确保令牌有效", "")]
    ...
    def _lorem_results(self, arg: str) -> list[ProviderResult]:
        ...
        for n in word_counts:
            ...
            results.append(self._make_result(f"{n} 个单词", words[:80] + "..." if len(words) > 80 else words, words))
        return results
    ...
    def _password_results(self, arg: str) -> list[ProviderResult]:
        ...
        results.append(self._make_result(pw_full, f"完整（{length} 字符）- 字母、数字、符号", pw_full))
        results.append(self._make_result(pw_alpha, f"字母数字（{length} 字符）- 字母、数字", pw_alpha))
        results.append(self._make_result(pw_hex, f"十六进制（{length} 字符）", pw_hex))
        results.append(self._make_result(passphrase, "口令 - 4 个随机单词", passphrase))
        return results
