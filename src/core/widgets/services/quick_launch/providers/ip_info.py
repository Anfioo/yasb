...
_TOOLS: dict[str, dict[str, str]] = {
    "info": {
        "name": "我的网卡",
        "description": "显示本地网络接口的 IP 与 MAC",
    },
    "public": {
        "name": "公网 IP",
        "description": "获取你的外部 IP 地址（在线）",
    },
    "calc": {
        "name": "子网计算器",
        "description": "根据 CIDR 计算网络、广播、主机范围",
    },
    "check": {
        "name": "IP 检查",
...
class IpInfoProvider(BaseProvider):
    ...
    def _calc_results(self, arg: str) -> list[ProviderResult]:
        ...
        try:
            ...
            return [self._make_result("无效的 CIDR 表示法", f"无法解析 '{arg}'", "")]
        ...
        if isinstance(network, ipaddress.IPv4Network):
            results.append(
                self._make_result(
                    f"广播：{network.broadcast_address}",
                    "点击复制",
    ...
    def _check_results(self, arg: str) -> list[ProviderResult]:
        ...
        try:
            ...
            return [self._make_result("无效的 IP 地址", f"无法解析 '{arg}'", "")]

        results: list[ProviderResult] = []
        results.append(self._make_result(str(addr), f"版本：IPv{addr.version}", str(addr)))

        if addr.is_private:
            results.append(self._make_result("类型：私有", "RFC 1918 私有地址", "私有"))
        elif addr.is_loopback:
            results.append(self._make_result("类型：回环", "回环地址", "回环"))
        elif addr.is_link_local:
            results.append(self._make_result("类型：链路本地", "链路本地地址", "链路本地"))
        elif addr.is_multicast:
            results.append(self._make_result("类型：组播", "组播地址", "组播"))
        elif addr.is_reserved:
            results.append(self._make_result("类型：保留", "保留地址", "保留"))
        else:
            results.append(self._make_result("类型：公网", "全球可路由地址", "公网"))
        ...
        if addr.version == 4:
            ...
            results.append(self._make_result(f"展开：{expanded}", "点击复制", expanded))
            int_val = int(addr)
            results.append(self._make_result(f"整数：{int_val}", "点击复制", str(int_val)))
    ...
    def _dns_results(self, arg: str) -> list[ProviderResult]:
        ...
        try:
            ...
            results.append(
                self._make_result(
                    f"DNS：{hostname}",
                    f"找到 {len(ipv4_addrs)} 个 IPv4、{len(ipv6_addrs)} 个 IPv6 地址",
                    hostname,
                )
            )
    ...
    def _mac_results(self, arg: str) -> list[ProviderResult]:
        ...
        if not mac_adapters:
            return [self._make_result("未找到 MAC 地址", "无法读取网卡信息", "")]
