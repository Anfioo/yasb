    ...
    try:
        ...
        if not is_pinned_only and win32gui.IsWindow(hwnd):
            ...
            if len(group_hwnds) > 1:
                ...
                menu.addAction("关闭所有窗口").triggered.connect(close_all)
...
def _add_chromium_browser_menu_items(menu: QMenu, identifier: str | None, on_launch_callback) -> None:
    ...
    if identifier:
        ...
        private_label = "新建隐身窗口" if is_chrome else "新建 InPrivate 窗口"

        menu.addAction("新窗口").triggered.connect(lambda _: on_launch_callback(identifier, extra_arguments=""))
...
def _add_firefox_browser_menu_items(menu: QMenu, identifier: str | None, on_launch_callback) -> None:
    ...
    if identifier:
        ...
        if is_uwp:
            ...
            menu.addAction("新窗口").triggered.connect(lambda _: on_launch_callback(identifier))
            ...
            menu.addAction("新窗口").triggered.connect(
...
def _add_vscode_menu_items(menu: QMenu, identifier: str | None, on_launch_callback) -> None:
    ...
    if identifier:
        menu.addAction("新窗口").triggered.connect(lambda _: on_launch_callback(identifier, extra_arguments="-n"))
        menu.addSeparator()
