...
class AboutDialog(ViewBase, QDialog):
    _STATE_CONFIG = {
        ...
        "available": {"text": "发现新版本", "enabled": True, "attr": "available"},
        "unsupported": {
            "text": "更新已禁用",
            "enabled": False,
            "attr": "unsupported",
    ...
    def _handle_up_to_date(self, _message: str, fetcher_ref: ReleaseFetcher | None = None) -> None:
        self._clear_fetcher(fetcher_ref)
        self._apply_state("idle", text_override="您已是最新版本", revert_after=2400)

    def _handle_check_failed(self, _message: str, fetcher_ref: ReleaseFetcher | None = None) -> None:
        self._clear_fetcher(fetcher_ref)
        self._apply_state("idle", text_override="更新检查失败", revert_after=3200)
