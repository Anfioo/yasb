    ...
    def run(self) -> None:
        try:
            ...
            if self._install_nerd:
                ...
                if self._stop:
                    self.finished.emit(False, "安装已取消。", nerd_paths)
                    return
            ...
            if self._install_segoe:
                if self._stop:
                    self.finished.emit(False, "安装已取消。", installed)
                    return
            ...
            try:
                ...
                pass
            self.finished.emit(True, "所有字体均已安装成功。", installed)
...
class WelcomeWizard(ViewBase, QDialog):
    ...
    def _page_fonts(self) -> QWidget:
        ...
        self._header(
            page_layout,
            "字体与图标",
            "YASB 需要特定字体来显示状态栏和 UI 图标。缺失的字体将自动下载并安装到你的用户账户。",
            spacing=16,
        )
        ...
        for font in REQUIRED_FONTS:
            ...
            text_layout.addWidget(title_label)
            desc = (
                "状态栏字体，包含 10,000+ 个流行图标集的图标"
                if "Nerd" in font["label"]
                else "用于 UI 元素的系统图标字体"
            )
            desc_label = TextBlock(desc, variant="caption")
            text_layout.addWidget(desc_label)
            row_layout.addLayout(text_layout)
    ...
    def _page_wm(self) -> QWidget:
        entries = [*WINDOW_MANAGER_GROUPS, ("none", "跳过", "我不使用平铺窗口管理器")]
        ...
        return self._card_grid_page(
            "平铺窗口管理器",
            "平铺窗口管理器会自动排列你的窗口。\nYASB 可以为 Komorebi 和 GlazeWM 显示工作区与控制按钮。如果你不使用，请选择跳过。",
            entries,
            2,
            on_wm,
            self._wm_cards,
    ...
    def _page_bar_options(self) -> QWidget:
        ...
        page_layout.setSpacing(0)
        self._header(
            page_layout, "显示与效果", "选择在哪块屏幕上显示状态栏，并配置视觉效果。"
        )

        screen_dd = DropDown(items=[("*", "所有屏幕"), ("primary", "主屏幕")])
        ...
        page_layout.addWidget(
            self._toggle_row(
                "启用模糊效果", "为状态栏背景应用毛玻璃模糊效果。", "blur", checked=True
            )
        )
    ...
    def _page_summary(self) -> QWidget:
        ...
        page_layout.setSpacing(0)
        page_layout.addStretch(2)
        title_label = TextBlock("全部就绪！", variant="title")
        title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        page_layout.addWidget(title_label)
        page_layout.addSpacing(12)
        tip_label = TextBlock(
    ...
    def _start_font_check(self) -> None:
        ...
        self._font_status.setText("正在检查环境要求...")
    ...
    def _write_config(
        ...
        logging.info("已在 %s 创建配置文件", HOME_CONFIG_PATH)
        ...
        logging.info("已在 %s 创建样式文件", HOME_STYLES_PATH)
